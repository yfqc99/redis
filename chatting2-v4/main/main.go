...
func main() {
	...
	for {
		...
		pubsub, user := getUser(conn)
		wg.Add(2)
		//发送
		go subscribe(pubsub, user)
		go accept(conn, user)
	}
	wg.Wait()
}
...
// 订阅消息
func subscribe(pubsub *redis.PubSub, user User) {
	//等待消息
	for {
		...
		message := msg.Payload
		if strings.Split(message, ":")[0] == user.name {
			continue
		}
...
// 得到客户端信息
func getUser(conn net.Conn) (*redis.PubSub, User) {
label:
	//接收第一次发送的消息
	reader := bufio.NewReader(conn)
...
func record(msg string) {
	filePath := "G:/goProject/src/chatting2-v4/document/record.txt"
